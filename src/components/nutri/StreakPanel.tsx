...
export function StreakPanel({ streak, history, todayAchieved }: StreakPanelProps) {
  ...
  const weekDays = Array.from({ length: 7 }).map((_, i) => {
    ...
    if (isDateToday) {
      ...
      isCompleted = historyItem?.waterGoalAchieved || false;
    }
  ...
  return (
    <Card className="p-4 mb-6 flex flex-row items-center justify-between overflow-hidden relative shadow-sm border border-kindra-800">
      {/* Glow vermelho sutil se a ofensiva estiver ativa */}
      {isFireActive && (
        <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[150px] h-[150px] bg-red-500/10 blur-2xl rounded-full pointer-events-none" />
      )}

      {/* Esquerda: Fogo + Número compactos */}
      <div className="flex items-center gap-3 z-10 relative">
        <div className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl transition-colors duration-500 ${isFireActive ? 'bg-red-500/10' : 'bg-kindra-800'}`}>
          <Flame 
            className={`w-5 h-5 sm:w-6 sm:h-6 transition-all duration-500 ${isFireActive ? 'text-red-500 fill-red-500 drop-shadow-md' : 'text-kindra-600'}`} 
            strokeWidth={2}
          />
        </div>
        <div className="flex flex-col">
          <div className="flex items-baseline gap-1">
            <span className={`text-2xl sm:text-3xl font-display font-bold tracking-tight transition-colors duration-500 ${isFireActive ? 'text-red-500' : 'text-kindra-400'}`}>
              {displayStreak}
            </span>
            <span className={`text-xs sm:text-sm font-bold tracking-wide transition-colors duration-500 hidden xs:block ${isFireActive ? 'text-red-600/80' : 'text-kindra-600'}`}>
              dias
            </span>
          </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 z-10">
        {weekDays.map((day, idx) => {
          ...
          if (day.isCompleted) {
            // Meta batida - fundo vermelho sutil
            circleStyle = "bg-red-500/10 border border-red-500/20";
          ...
          return (
            <div key={idx} className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center transition-all duration-300 ${circleStyle}`}>
              <Flame 
                className={`w-3 h-3 sm:w-4 sm:h-4 transition-all ${day.isDateToday ? 'text-red-500 fill-red-500' : 'text-kindra-500'}`} 
                strokeWidth={2} 
              />
            </div>
          );
